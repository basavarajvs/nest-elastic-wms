...
export class WmsAbilityFactory {
  createForUser(jwtPayload: Record<string, any>): WmsAbility {
    ...
    if (roles.includes('TENANT_ADMIN')) {
      ...
      return build({
        detectSubjectType: (item: any) =>
          typeof item === 'string'
            ? item
            : item.__caslSubjectType__ || (item.constructor?.name as any),
      });
    }
    ...
    for (const roleDef of WMS_ROLE_DEFINITIONS) {
      if (roles.includes(roleDef.roleCode)) {
        for (const perm of roleDef.permissions) {
          const subject = perm.subject === 'all' ? 'all' : (perm.subject as any);
          can(perm.action, subject);
          if (perm.action === WmsAction.Manage && subject !== 'all') {
            can(
              [WmsAction.Create, WmsAction.Read, WmsAction.Update, WmsAction.Delete],
              subject,
            );
          }
        }
